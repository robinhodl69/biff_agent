...
export const config = {
  ...
  AGENT_LOOP_INTERVAL_MS:
    (Number(process.env.AGENT_LOOP_INTERVAL_MIN) || 5) * 60 * 1000,
  API_PORT: Number(process.env.API_PORT) || 3000,
  ADMIN_SECRET: process.env.ADMIN_SECRET || "biff-admin-secret-change-me",

  // Floe Protocol - Base Sepolia
  FLOE_API_URL: process.env.FLOE_API_URL || "https://api.floe.finance/v1",
  LENDING_INTENT_MATCHER:
    "0xF351eDF229ded7E2e2b23E44c70e9964CbA91B2E" as `0x${string}`,
  ...
  CHAINLINK_WETH_USD_FEED: (() => {
    ...
    if (!isAddress(addr))
      throw new Error(`Invalid CHAINLINK_WETH_USD_FEED: ${addr}`);
