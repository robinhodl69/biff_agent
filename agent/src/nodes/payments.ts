...
export async function repayOrRenew(state: BiffState): Promise<Partial<BiffState>> {
  ...
  try {
    const expiringLoan = state.activeLoans.find(l => l.daysRemaining < config.LOAN_WARN_DAYS)
    ...
    if (shouldRepay) {
      ...
      return { lastAction: 'repaid', actionReason: `Préstamo ${expiringLoan.id} pagado.` }
    }

    return { lastAction: 'idle', actionReason: 'Renewal logic pending Phase 6' }
  } catch (error: any) {
